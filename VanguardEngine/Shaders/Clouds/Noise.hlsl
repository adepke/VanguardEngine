...
float PerlinNoise(float3 p, float frequency, float octaves)
{
	...
	for (uint i = 0; i < baseSize; ++i)
	{
		for (uint j = 0; j < baseSize; ++j)
		{
			...
			float perlinWorley = 0.f;
			{
				...
				const float worleyNoise0 = (1.0 - WorleyNoise(coord, cellCount * noiseFrequencies[0]));
				const float worleyNoise1 = (1.0 - WorleyNoise(coord, cellCount * noiseFrequencies[1]));
				const float worleyNoise2 = (1.0 - WorleyNoise(coord, cellCount * noiseFrequencies[2]));
			...
			{
				const float cellCount = 4;
				const float worleyNoise1 = (1.0 - WorleyNoise(coord, cellCount * 2));
				const float worleyNoise2 = (1.0 - WorleyNoise(coord, cellCount * 4));
				const float worleyNoise3 = (1.0 - WorleyNoise(coord, cellCount * 8));
				const float worleyNoise4 = (1.0 - WorleyNoise(coord, cellCount * 16));
...
void DetailShapeMain(uint3 dispatchId : SV_DispatchThreadID)
{
	...
	for (uint i = 0; i < detailSize; ++i)
	{
		for (uint j = 0; j < detailSize; ++j)
		{
			...
			const float worleyNoise0 = (1.0 - WorleyNoise(coord, cellCount * 1));
			const float worleyNoise1 = (1.0 - WorleyNoise(coord, cellCount * 2));
			const float worleyNoise2 = (1.0 - WorleyNoise(coord, cellCount * 4));
			const float worleyNoise3 = (1.0 - WorleyNoise(coord, cellCount * 8));
