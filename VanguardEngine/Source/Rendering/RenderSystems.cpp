	...
	Registry.view<const TransformComponent, const CameraComponent>().each([](auto Entity, const auto& Transform, const auto& Camera)
		{
			...
			const auto UpDirection = XMVectorSet(Camera.UpDirection.x, Camera.UpDirection.y, Camera.UpDirection.z, 0.f);
			...
			GlobalViewMatrix = XMMatrixTranspose(ViewMatrix);
			GlobalProjectionMatrix = XMMatrixTranspose(ProjectionMatrix);
		});
}