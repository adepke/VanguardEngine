...
std::shared_ptr<Texture> TextureLoader::Load(std::filesystem::path Path)
{
	int PixelsX;
	int PixelsY;
	...
	auto TextureResource = Renderer::Get().Device->CreateResource(Description, VGText("Asset Texture"));

	Renderer::Get().Device->WriteResource(TextureResource, DataResource);
