...
private:
    ...
    template<typename U>
    size_t hash_object( U && key)
    {
        return static_cast<Hasher &>(*this)(std::forward<U>(key));
    }
    template<typename U>
    size_t hash_object( U && key) const
    {
        return static_cast<const Hasher &>(*this)(std::forward<U>(key));
    }
